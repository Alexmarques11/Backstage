...
deploy_application() {
    ...
    # Verificar se secrets existem (mas não criar automaticamente)
    if ! kubectl get secret backstage-secrets -n $NAMESPACE &> /dev/null; then
        log_error "backstage-secrets not found!"
        log_error "Please create secrets manually first:"
        log_error "1. Generate passwords: openssl rand -hex 32"
        log_error "2. Create secrets: kubectl create secret generic backstage-secrets ..."
        log_error "3. See SECURITY.md for complete instructions"
        exit 1
    fi
    
    if ! kubectl get secret postgres-secret -n $NAMESPACE &> /dev/null; then
        log_error "postgres-secret not found!"
        log_error "Please create postgres-secret manually first (see SECURITY.md)"
        exit 1
    fi
    
    log_success "Required secrets found - proceeding with deployment"
